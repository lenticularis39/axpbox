...
size_t CDiskDevice::read_bytes(void *dest, size_t bytes) {
  ...
  if (r != (byte_len)) {
    printf("%s: Tried to read %lu bytes from pos %lld, but could only read %lu bytes!\n",
...
size_t CDiskDevice::write_bytes(void *src, size_t bytes) {
  ...
  if (byte_from != state.byte_pos) {
    ...
    if (r != (dev_block_size)) {
      printf("%s: Tried to read %zu bytes from pos %lld, but could only read %lu bytes!\n",
  ...
  if ((byte_to != state.byte_pos + bytes) &&
    ...
    ReadFile(handle, buffer + byte_len - dev_block_size, (DWORD)dev_block_size,
             &r, NULL);
    if (r != (dev_block_size)) {
      printf("%s: Tried to read %zu bytes from pos %zd, but could only read %lu bytes!\n",
  ...
  if (r != byte_len) {
    printf("%s: Tried to write %lu bytes to pos %zd, but could only write %lu bytes!\n",
           devid_string, byte_len, byte_from, r);
